function special(mname,te,T2s,np,sw,freq_rf,freq_rcv,ideal_pulse,pw)
...
U3=pulse_U(s,'spectrum_simulation/P10.RF',pw(3),1.67*2/pw(3),270,H0_rf);
...
save([mname,'_special']);
fprintf('Results saved in %s_special.mat\n',mname);
figure;
ph=plot_sp(mean(data,2),freq_rcv,sw*1e6/s.f0,1);
write_mrui2(-mean(data*exp(1i*ph),2),[mname,'_special'],sw);
    
fprintf('Simulated spectrum saved in %s_special.mrui\n',mname); 
